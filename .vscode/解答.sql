...
FROM "SKILL" s
INNER JOIN "COACH_LINK_SKILL" cls ON s.id = cls.skill_id
...
SELECT 
  cp.name as 組合包方案名稱,
  COUNT(crp.id) as 銷售數量
FROM "CREDIT_PACKAGE" cp
LEFT JOIN "CREDIT_PURCHASE" crp ON cp.id = crp.credit_package_id
WHERE crp.created_at >= '2024-11-01' AND crp.created_at < '2024-12-01'
...
FROM "CREDIT_PURCHASE"
WHERE purchase_at >= '2024-11-01' AND purchase_at < '2024-12-01';
...
FROM "COURSE_BOOKING"
WHERE 
  created_at >= '2024-11-01' AND created_at < '2024-12-01'
  AND status != '課程已取消';
